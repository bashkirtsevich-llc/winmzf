...
//Decode        
      ...
      //error(tempfilename);
        If (TextInList(TempFileName,ExtractList)Or ExtractAll) Then   //If want extract
          Begin
            //If encode mode
            If (AConfig.EncodeFile) And ((AConfig.EncodeMode=emAfterCompress)Or
                                   ((AConfig.EncodeMode=emBeforeAfter))) Then
              Begin
                If Pass='' Then
                  GetPassword;
            ...
            EtalonFile.CopyFrom(Temp,Temp.Size);
            Temp.Free;
            If (AConfig.EncodeFile) And ((AConfig.EncodeMode=emBeforeCompress)Or
                                   ((AConfig.EncodeMode=emBeforeAfter))) Then
              Begin
                If Pass='' Then
                  GetPassword;
