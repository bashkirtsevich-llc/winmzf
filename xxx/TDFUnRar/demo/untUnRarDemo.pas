...
type
  TfrmMain = class(TForm)
    ...
    procedure dfUnRarProgress(Sender: TObject; FilesProcessed, FileCount,
...
begin
  ...
  dfUnRar.OnComment:=dfUnRarComment;
  dfUnRar.OnOverride:=dfUnRarOverride;
  dfUnRar.OnFileProcessing:=dfUnRarFileProcessing;
  dfUnRar.OnError:=dfUnRarError;
  dfUnRar.OnRarStatus:=dfUnRarRarStatus;
  IsRarLoaded  := false;
  IsExtracting := false;
...
procedure TfrmMain.dfUnRarPassword(Sender: TObject; var Password: String);
var Temp:String;
begin
  If InputQuery('Password','Write password',Temp) Then
    Password:=Temp;
end;
