...
begin
  ...
  assign(ptf,'sample1.exe');  reset(ptf,1);
  assign(ctf,'sample1.enc');  rewrite(ctf,1);
  EncryptFile(ptf, ctf, Samplekey, SampleIV);

  writeln('Decrypting ...');
  assign(ctf,'sample1.enc');  reset(ctf,1);
  assign(ptf,'sample9.exe');  rewrite(ptf,1);
