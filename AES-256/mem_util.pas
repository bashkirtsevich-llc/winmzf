...
procedure Hex2Mem({$ifdef CONST}const{$endif} s: string; pdest: pointer; MaxL: word; var L: word);
...
function  Base64EncStr({$ifdef CONST}const{$endif} s: string): string;
  {-Simple Base64 encoder, uses Base64Str}

function  Base64DecStr({$ifdef CONST}const{$endif} es: string): string;
...
const
  CT64: array[0..63] of char = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/';
...
begin
  ...
  for i:=1 to length(s) do begin
    ...
    if p>=0 then begin
      ...
      {ignore white space}
      if pos(c,#32#9#13#10)=0 then goto _break;
    end;
  end;
...
begin
  ...
  for i:=1 to length(es) do begin
    ...
    if bits>7 then begin
      ...
      dec(bits,8);
      result := result + char((buf shr bits) and $FF);
    end;
  end;
