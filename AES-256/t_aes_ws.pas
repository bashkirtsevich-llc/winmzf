{-Speed test prog for AES modes, we 2003}
...
begin
  OK := true;
  OK := OK and Test(hex32, hex32, hex32, hex32, hex32, tag00,  0,  0,  0);
  OK := OK and Test(hex32, hex32, hex32, hex32, hex32, tag01, 16,  0,  0);
  OK := OK and Test(hex32, hex32, hex32, hex32, hex32, tag02,  0, 16,  0);
  OK := OK and Test(hex32, hex32, hex32, hex32,  ct03, tag03, 16, 16, 32);
  OK := OK and Test(hex32, hex32, hex32, hex32,  ct04, tag04, 15, 14, 29);
  OK := OK and Test(key05, hdr05, non05, hex32, hex32, tag05, 16, 08,  0);
  OK := OK and Test(key06, hdr06, non06,  pt06,  ct06, tag06, 16, 08,  2);
  OK := OK and Test(key07, hdr07, non07,  pt07,  ct07, tag07, 16, 08,  5);
  OK := OK and Test(key08, hdr08, non08,  pt08,  ct08, tag08, 16, 08,  5);
  OK := OK and Test(key09, hdr09, non09,  pt09,  ct09, tag09, 16, 08,  6);
  OK := OK and Test(key10, hdr10, non10,  pt10,  ct10, tag10, 16, 08, 12);
  OK := OK and Test(key11, hdr11, non11,  pt11,  ct11, tag11, 16, 08, 17);
  OK := OK and Test(key12, hdr12, non12,  pt12,  ct12, tag12, 16, 08, 18);
  OK := OK and Test(key13, hdr13, non13,  pt13,  ct13, tag13, 16, 08, 18);
  OK := OK and Test(key14, hdr14, non14,  pt14,  ct14, tag14, 16, 08, 21);
  write('EAX  test: ');
...
begin
  {$ifdef USEDLL}
    writeln('Test program for AES_DLL V',AES_DLL_Version,'   (C) 2004-2007  W.Ehrhardt');
  {$else}
    writeln('Test program for AES functions    (C) 2004-2007  W.Ehrhardt');
  {$endif}
