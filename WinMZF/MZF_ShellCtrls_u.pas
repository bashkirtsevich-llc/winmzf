...
unit MZF_ShellCtrls_u platform;

interface

uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
  ComCtrls, CommCtrl, ShlObj, ActiveX, StdCtrls, ImgList, SignatureDetect_u,
  Advanced;
...
{ TMadDirBrowser }
  ...
  TMadDirBrowser = class(TCustomShellTreeView)
  published
...
{ TMZFDirCombo }
  ...
  TMZFDirCombo = class(TCustomShellComboBox)
  published
    property Images;
    property Root;
...
{ TMadFileBrowser }
  ...
  TMadFileBrowser = class(TCustomShellListView)
  published
    property AutoContextMenus;
    property AutoRefresh;
...
uses ShellConsts, ShellAPI, ComObj, TypInfo, Menus, Consts, Math, Main_u;
  ...
  if FAutoNavigate and (Selected <> nil) then
    with Folders[Selected.Index] do
      if IsFolder then
        ...
        FType:=GetArchiveTypeBySignature(PathName);
          If FType=ftAnyFile Then
            ShellOpenFile(Self.Parent.Handle,PathName)
          Else ShellOpenArchive(PathName,FType);
        end;
  inherited DblClick;
end;
      ...
      VK_RETURN:
        ...
        begin
          ...
          begin
            ...
            FType:=GetArchiveTypeBySignature(SelectedFolder.PathName);
              If FType=ftAnyFile Then
                ShellOpenFile(Parent.Handle,SelectedFolder.PathName)else
                ShellOpenArchive(SelectedFolder.PathName,FType);
            end;
      VK_BACK: if not IsEditing then Back;
      VK_F5: Refresh;
      //VK_DELETE:DeleteFile(SelectedFolder.AbsoluteID);
    end;
end;
