...
Type TMZFDecompressor=class (TComponent)
  ...
  Public
    ...
    procedure Extract(ArchiveStream:TMemoryStream);
    procedure Free;
    destructor Destroy;
  Protected
  End;
...
procedure TMZFDecompressor.Extract(ArchiveStream:TMemoryStream);  //Note!!! Delete text from List: "Root-[%s]"
         ...
         End Else
          Begin
            ...
            If Not fSecurityMode Then
              If Not NameInMask(fExcludeSpec,ExtractFileName(FileHead.LongFileName)) Then
                Begin
                  EtalonFile.SaveToFile(Dir+TempFileName);
                  SetFileAttributes(PChar(Dir+TempFileName),FileHead.Attr);
                  SetFileDateTime(Dir+TempFileName,
