...
Begin
  ...
  If Err<>0 then
    Begin
      ...
      MessageBox(dlgExtract.Handle,PChar(Format('Encrypt initialize error, code "%x"',[Err])),'Error',MB_OK+MB_ICONHAND);
      Exit;
    End;
  ...
  While Instream.Position<>InStream.Size Do
    Begin
      ...
      If err<>0 Then
        Begin
          //writeln(format('Error %x',[err]));
          MessageBox(dlgExtract.Handle,PChar(Format('Encrypt error, code "%x"',[Err])),'Error',MB_OK+MB_ICONHAND);
          Exit;
        End;
