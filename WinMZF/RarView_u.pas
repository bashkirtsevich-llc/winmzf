...
Uses SysUtils, DFUnRar, Classes, Advanced, Viewer_u, Windows;
...
Type TRarViewer=Class(TComponent)
  ...
  Public
    ...
    property OnShowFileInfo:TSowFileInfo read fOnShowInfo write fOnShowInfo;
...
Var FileInfo:TFileHead;
    fAttr:Word;
begin
  If (FilterDir<>'%ALL%')And(UpperCase(ExtractFileDir(hdrData.FileName))<>UpperCase(FilterDir)) Then Exit;
  {showmessage(filterdir);
  showmessage(ExtractFileDir(hdrData.FileName));}
  If hdrData.FADirectory Then Exit;
  fAttr:=0;
  if hdrData.FAArchive    then FAttr := FAttr + FILE_ATTRIBUTE_ARCHIVE;
  if hdrData.FACompressed then FAttr := FAttr + FILE_ATTRIBUTE_COMPRESSED;
  if hdrData.FAHidden     then FAttr := FAttr + FILE_ATTRIBUTE_HIDDEN;
  if hdrData.FANormal     then FAttr := FAttr + FILE_ATTRIBUTE_NORMAL;
  if hdrData.FAOffLine    then FAttr := FAttr + FILE_ATTRIBUTE_OFFLINE;
  if hdrData.FAReadOnly   then FAttr := FAttr + FILE_ATTRIBUTE_READONLY;
  if hdrData.FASystem     then FAttr := FAttr + FILE_ATTRIBUTE_SYSTEM;
  if hdrData.FATempporary then FAttr := FAttr + FILE_ATTRIBUTE_TEMPORARY;
  //
  FileInfo.LongFileName:=hdrData.FileName;
  FileInfo.Size:=hdrData.UnpSize;
  FileInfo.PackedSize:=hdrData.PackSize;
  FileInfo.CRC:=StrToInt('$'+hdrData.FileCRC);
  FileInfo.FileCreateDate:=hdrData.FileTime;
  FileInfo.FileModifyDate:=hdrData.FileTime;
  FileInfo.FileOpenDate:=hdrData.FileTime;
  FileInfo.Attr:=fAttr;
  If Assigned(fOnShowInfo) Then
    fOnShowInfo(Self,FileInfo,0);
end;
